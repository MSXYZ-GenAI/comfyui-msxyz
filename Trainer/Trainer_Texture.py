...
# Trains the 1x texture refinement model used by comfyui-solricks.
...
def degrade_input(x, strong=True):
    ...
    # Strong blur so the model learns to restore texture.
    if random.random() < 0.8:
        sigma = random.uniform(0.5, 2.0) if strong else random.uniform(0.2, 1.0)
        x = transforms.functional.gaussian_blur(x, kernel_size=(5,5), sigma=sigma)

    # Resolution loss through downscale and upscale.
    scale = random.uniform(0.4, 0.6) if strong else random.uniform(0.7, 0.9)
    nh, nw = max(16, int(h * scale)), max(16, int(w * scale))
    
    # Pick a random downscale mode for varied texture loss.
    down_mode = random.choice(["bilinear", "bicubic", "area"])
    y = F.interpolate(x, size=(nh, nw), mode=down_mode, align_corners=False if down_mode != "area" else None)
    
    # Upscale back to the original size, leaving a slightly blurred image.
    y = F.interpolate(y, size=(h, w), mode="bilinear", align_corners=False)

    # Light noise so the model sees texture under mild degradation.
    if random.random() < 0.3:
        noise = torch.randn_like(y) * random.uniform(0.001, 0.005)
...
def save_inference_weights(model, out_dir, name="DLAATexture", export_fp16=True):
    out_dir = Path(out_dir)
    out_dir.mkdir(parents=True, exist_ok=True)
    ...
    for epoch in range(start_epoch, args.epochs + 1):
        ...
        if is_best:
            ...
            if pth_size > args.max_model_mb:
                print(f"[WARN] DLAATexture.pth is {pth_size:.2f} MB, above limit {args.max_model_mb:.2f} MB")

            if safe_size > args.max_model_mb:
                print(f"[WARN] DLAATexture.safetensors is {safe_size:.2f} MB, above limit {args.max_model_mb:.2f} MB")

            print(
                f"[BEST] val={best_val:.6f} | "
                f"DLAATexture.pth={pth_size:.2f} MB | "
                f"DLAATexture.safetensors={safe_size:.2f} MB | "
...
def parse_args():
    parser = argparse.ArgumentParser(description="DLAATexture Trainer")
    ...
    parser.add_argument("--w-temporal", type=float, default=0.2)
    parser.add_argument("--w-perceptual", type=float, default=0.4)
