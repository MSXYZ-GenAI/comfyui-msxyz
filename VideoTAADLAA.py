...
class _TAAState:
    ...
    def update(self, frame: torch.Tensor, alpha: float, sensitivity: float) -> torch.Tensor:
        ...
        flow = self._optical_flow(self.prev_frame, frame)
        ...
        sample_grid = (base_grid.unsqueeze(0) + flow).clamp(-1, 1)
    ...
    def execute(self, images, taa_alpha, motion_sensitivity, sharpen_strength, reset_history=True):
        ...
        with torch.no_grad():
            for i in range(B):
                ...
                out_frames.append(rgb.permute(0, 2, 3, 1).cpu())
